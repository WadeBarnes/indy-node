...
>Important: The “client” IP address referred to here is not the CLI machine’s IP address. Reminder:  The Validator node has a node IP address for communications with other Validators and a “client” IP address for communications with edge agents (anything outside the Sovrin Network of Validators).
